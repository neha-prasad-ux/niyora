...
// - No event meta is forwarded. PostHog receives the event name, an
//   anonymous random id, the app version, and the OS string. Nothing else.
...
// absent: stress check-in data never leaves the Mac.
...
/// listed here is dropped before the event leaves the device. Notably
/// `reminder_fired` keeps `interval_min`/`snoozed` but drops `score` and
/// `label` — the stress reading itself never leaves the Mac.
