...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // First check delayed a few seconds so it never blocks tray-icon
            // registration or first paint of the panel. Then re-checks every
            // 24h while the app stays running, since this is a tray app users
            // rarely relaunch.
            {
                ...
                std::thread::spawn(move || {
                    ...
                    loop {
                        updater::check(handle.clone(), false);
