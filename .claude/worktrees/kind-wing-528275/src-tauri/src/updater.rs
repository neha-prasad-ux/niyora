...
//
// Behavior:
// - On launch, `check(app, false)` is called once (after a few seconds), then
//   re-runs every 24h while the app stays running. If a new version is found,
//   the user gets a notification with "Update now" / "Later". No notification
//   is shown if they're already on the latest version. The 24h interval matters
//   because this is a tray app — users rarely relaunch, so a launch-only check
//   would leave long-running instances stuck on stale builds.
